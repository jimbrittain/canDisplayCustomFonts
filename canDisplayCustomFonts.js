"use strict";
/*globals fetter, window, findStyleSheet, addStyleSheet, setTimeout, isBrowserLegacyIE, findElementsByTag, document, setAttribute, findElementStyle, isHTMLElement, addStyleSheetRule, findElementById, isString */
/**
  @module DisplayCustomFonts
  @class DisplayCustomFonts
  @singleton
  @description - self initialises on construction;
 **/
var DisplayCustomFonts = function(){
        if(DisplayCustomFonts.prototype.singleton !== undefined){ return DisplayCustomFonts.prototype.singleton; }
        DisplayCustomFonts.prototype.singleton = this;
        this.tested = false;
        this.name = "imwebfonttester";
        this.supports = {
            'eot': false,
            'woff': false,
            'svg' : false,
            'otf' : false,
            'ttf' : false };
        /**
          @description data-uris now include format to prevent browser errors
         **/
        this.datauris = {
            'eot' : "url('data:application/vnd.ms-fontobject;base64,') format('embedded-opentype')",
            'woff' : "url('data:application/font-woff;base64,') format('woff')",
            'svg' : "url('data:image/svg+xml;base64,') format('svg')",
            'otf' : "url('data:application/x-font-opentype;base64,') format('opentype')",
            'ttf' : "url('data:application/x-font-truetype;base64,') format('truetype')"
        };
        this.init(); };
/**
  @method DisplayCustomFonts.init
  @description -creates test procedures after DOMContentLoaded
  @requires util.tools.fetter
  @develop -issue on DOMContentLoaded:
 **/
DisplayCustomFonts.prototype.init = function(){
    var c = this;
    fetter(window, 'load', [c, function(){ 
        if(findStyleSheet({name: 'candisplaycustomfonts'}) === false){
            addStyleSheet({name: 'candisplaycustomfonts'}); }
        setTimeout(function(){ c.test();}, 500); }], true, 'after'); };
/**
  @method DisplayCustomFonts.test
  @description -builds test elements and stylesheet rules for the tests
 **/
DisplayCustomFonts.prototype.test = function(){
    if(isBrowserLegacyIE('LT9')){
        this.supports.eot = true;
    } else {
        var bodyTag = findElementsByTag('body')[0],
            sheet = findStyleSheet({name: 'candisplaycustomfonts'}),
            c = this;
        for(var prop in this.datauris){
            if(this.datauris.hasOwnProperty(prop)){
                var rule = "font-family: '"+ this.name + prop + "'; src: " + this.datauris[prop] + ';';
                var added = false;
                try {
                    added = addStyleSheetRule('@font-face', rule, sheet);
                } catch(e){ 
                    added = false; }
                if(added){
                    if('createElement' in document){
                        var elem = document.createElement('div');
                        setAttribute(elem, 'id', this.name + prop);
                        setAttribute(elem, 'style', 'visibility:hidden;position:absolute;bottom:0;right:0;padding:0;margin:0;font-size:20px;' + "font-family:'" +this.name+prop+"',serif;");
                        elem.textContent = '---';
                        bodyTag.appendChild(elem);
                    } else if('innerHTML' in bodyTag){
                        bodyTag.innerHTML += '<div id="' + this.name + prop + '" style="visibility:hidden;position:absolute;bottom:0;right:0;padding:0;margin:0;font-size:20px;' + "font-family:'" +this.name+prop+"',serif;" + '">---</div>'; }
                    this.establishTest(prop); }
            }
        }
    }
    this.tested = true; };
/**
  @method DisplayCustomFonts.establishTest
  @param type {String} from this.datauri[prop] where prop === type
  @description -based on PI advising time test
 **/
DisplayCustomFonts.prototype.establishTest = function(type){ var c = this, t = setTimeout(function(){ c.testReturn(type); }, 150); };
/**
  @method DisplayCustomFonts.testReturn
  @param type {String} from this.datauri[prop] where prop === type
 **/
DisplayCustomFonts.prototype.testReturn = function(type){
    var elem = findElementById(this.name + type),
        bodyTag = findElementsByTag('body')[0];
    if(isHTMLElement(elem)){
        if(parseFloat(findElementStyle(elem, 'offsetWidth')) < 10){
            this.supports[type] = true; }
        if('removeChild' in bodyTag){ 
            bodyTag.removeChild(elem); 
        } else { elem.style.display = 'none'; }}};
/**
  @method DisplayCustomFonts.doesSupport
  @param type {String} from this.datauri[prop] where prop === type/type === file extension, if undefined test if any
  @return {Boolean}
  @alias util.tools.canDisplayCustomFonts;
 **/
DisplayCustomFonts.prototype.doesSupport = function(type){
    if(this.tested){
        if(isString(type)){
            type = type.toLowerCase();
            if(type in this.supports){
                return this.supports[type]; }}
        return this.doesSupportAny(); }};
/**
 * @method DisplayCustomFonts.doesSupportAny
 * @return {Boolean}
 * @description -tests if any webfonts give a valid result
 **/
DisplayCustomFonts.prototype.doesSupportAny = function(){
    if(this.tested){
        for(var prop in this.supports){
            if(this.supports.hasOwnProperty(prop) && this.supports[prop] === true){ return true; }}
        return false; }};
var canDisplayCustomFonts = function(str){ return (new DisplayCustomFonts()).doesSupport(str); };
var displayCustomFonts = new DisplayCustomFonts();
